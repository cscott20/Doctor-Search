...
class Doctor:
    def __init__(self, npi, cigna_name, org):
        ...
        root = "https://www.uhhospitals.org/doctors/"
        self.npi = npi
        self.org = org
        ...
        try: 
            ...
            start = time.time()
            #Get "soup" HTML Doc
            self.res = requests.get(self.profile_link)
            res = self.res.text
            end = time.time()
    ...
    def check_in_dir(self):
        ...
        if self.res.url != self.profile_link:
            print(self.profile_link)
        if self.res.url == self.profile_link and message != error:
